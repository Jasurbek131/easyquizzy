                    ...
                    if ($response['status']) {
                        ...
                        if (!empty($plannedStops)) {
                            foreach ($plannedStops as $stop) {
                                ...
                                if ($item['id'])
                                    $planStop = PlmStops::findOne($stop['planned_stop_id']);
                        ...
                        if (!empty($unplannedStops)) {
                            foreach ($unplannedStops as $unStop) {
                                ...
                                if ($item['unplanned_stop_id'])
                                    $unPlanStop = PlmStops::findOne($unStop['unplanned_stop_id']);
                                $unPlanStop->setAttributes([
                                    'document_item_id' => $docItem->id,
                                    'reason_id' => $unStop['reason_id'],
                        ...
                        ->select([
                            ...
                            'notifications_status' => function($ns){
                                $ns->from(['pnl' => "plm_notifications_list"])
                                    ...
                                    ->where(["pnl.status_id" => BaseModel::STATUS_ACTIVE]);
                            }
